...
export const deviceApi = {
  getStatus: () => fetch(`${API_BASE_URL}/api/device/status`).then(r => r.json()),
  ...
  disconnect: () => 
    fetch(`${API_BASE_URL}/api/device/disconnect`, { method: 'POST' }).then(r => r.json()),
  ...
  signTransaction: () =>
    fetch(`${API_BASE_URL}/api/transaction/sign`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
